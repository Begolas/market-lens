...
async function fetchCandles(symbol, interval, apiKey) {
  ...
  let params;
  if      (interval==="1D")  params = {function:"TIME_SERIES_DAILY_ADJUSTED", symbol, outputsize:"full", apikey:apiKey};
  else if (interval==="1W")  params = {function:"TIME_SERIES_WEEKLY_ADJUSTED", symbol, apikey:apiKey};
  else if (interval==="1Mo") params = {function:"TIME_SERIES_MONTHLY_ADJUSTED", symbol, apikey:apiKey};
  else                        params = {function:"TIME_SERIES_INTRADAY", symbol, interval, outputsize:"full", apikey:apiKey, adjusted:"true"};
  const data = await avFetch(params);
  if (data["Note"]||data["Information"]) throw new Error("Rate limit – bitte 1 Min. warten");
...
const INTERVALS = ["5min","15min","30min","60min","1D","1W","1Mo"];
const IV_LABEL  = {"5min":"5M","15min":"15M","30min":"30M","60min":"1H","1D":"1T","1W":"1W","1Mo":"1Mo"};
